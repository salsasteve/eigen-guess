...
def low_rank_approximate(mat_org: torch.tensor, rank=32):
    ...
    output = {"mat_q": mat_q, "mat_r": mat_r.t(), "error": error}
    return output
...
class ModuleLowRank(object):
    ...
    def __init__(self, compress_ratio=3, name_omit=list(), is_approximate=True):
        super().__init__()
    ...
    def _apply(self, name: str, module: nn.Linear):
        ...
        rank = (shape[0] * shape[1]) // (
            self.compress_ratio * (shape[0] + shape[1])
        )
        rank = int(rank)
    ...
    def __call__(self, module: nn.Module):
        copied_modules = {
            name: module_sub for name, module_sub in module.named_modules()
        }
